"""XGBoost residual corrector: tree model trained on base model residuals.

Fundamentally different from the NN residual:
- Gradient boosting naturally handles tabular feature interactions.
- Tree-based learning is more robust than MLP optimization on residuals.
- Same feature selection logic as the base XGB model.
- Uses MSE objective by default (residuals are roughly zero-mean).
"""
...
class XGBResidualModel:
    def __init__(self):
        self.max_rows = int(os.environ.get("MEOW_XGBR_MAX_ROWS", "500000"))
        self.max_depth = int(os.environ.get("MEOW_XGBR_MAX_DEPTH", "5"))
        self.learning_rate = float(os.environ.get("MEOW_XGBR_LEARNING_RATE", "0.05"))
        self.n_estimators = int(os.environ.get("MEOW_XGBR_N_ESTIMATORS", "100"))
        self.subsample = float(os.environ.get("MEOW_XGBR_SUBSAMPLE", "0.8"))
        self.colsample_bytree = float(os.environ.get("MEOW_XGBR_COLSAMPLE_BYTREE", "0.8"))
        self.objective = os.environ.get(
            "MEOW_XGBR_OBJECTIVE", "reg:squarederror"
        ).strip()
        self.exclude_families = {
            f.strip()
            for f in os.environ.get(
                "MEOW_XGBR_EXCLUDE_FAMILIES",
    ...
    def partial_fit(self, xdf, resid):
        ...
        resid is a 1-D array of base model residuals (fret12 - base_pred).
        """
        ...
        r = np.asarray(resid, dtype=np.float32).ravel()

        # Track group IDs only for ranking objectives
        if self.objective.startswith("rank:"):
            date_vals = xdf.index.get_level_values("date").to_numpy(dtype=np.int64)
            intv_vals = xdf.index.get_level_values("interval").to_numpy(dtype=np.int64)
        ...
        if self._X_reservoir is None:
            ...
            self._y_reservoir = r[:take].copy()
            if group is not None:
                self._group_ids = group[:take].copy()
            ...
            if capacity < self.max_rows:
                ...
                self._X_reservoir = np.concatenate(
                    [self._X_reservoir, x[:take]], axis=0
                )
                self._y_reservoir = np.concatenate(
                    [self._y_reservoir, r[:take]], axis=0
                )
                if group is not None:
                    self._group_ids = np.concatenate(
                        [self._group_ids, group[:take]], axis=0
                    )
            else:
                for i in range(n):
    ...
    def _keep_column(self, name):
        if self.exclude_patterns and any(
            pattern in name for pattern in self.exclude_patterns
        ):
            return False
