...
class RankLGBModel:
    ...
    def partial_fit(self, xdf, ydf):
        ...
        # Extract group keys from MultiIndex: (date, interval)
    ...
    def finalize_fit(self):
        if self._X_reservoir is None or len(self._y_reservoir) < 1000:
            return
        # Sort by group key to make groups contiguous (required by lambdarank)
        ...
        # Compute group sizes: count consecutive rows with same (date, interval)
        groups = []
        prev = None
        count = 0
