...
N_RAW = len(RAW_LOB_COLS)  # 23


def _extract_normalized(raw_df, eps=1e-8):
    """Extract raw LOB columns and normalize to ratios/z-scores.
    ...
    bid_total = (raw_df["bsize0_4"].to_numpy(dtype=np.float64, copy=False)
                + raw_df["bsize5_9"].to_numpy(dtype=np.float64, copy=False)
                + raw_df["bsize10_19"].to_numpy(dtype=np.float64, copy=False))
    ask_total = (raw_df["asize0_4"].to_numpy(dtype=np.float64, copy=False)
    ...
    tr_total = (raw_df["btr0_4"].to_numpy(dtype=np.float64, copy=False)
    ...
    # Log midpx (1 col) — absolute price level info
...
class RawLobMLP(nn.Module):
    ...
    Architecture: 23 → 64 → 32 → 1  (3,521 parameters)
