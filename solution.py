...
def _add_rks_features(xdf, rks):
    ...
    rks_feats = rks.transform(x_base)
    rks_names = [f"rks_{i}" for i in range(rks_feats.shape[1])]
    rks_df = pd.DataFrame(rks_feats, index=xdf.index, columns=rks_names)
    return pd.concat([xdf, rks_df], axis=1)
...
def train_and_evaluate(h5dir: Optional[str] = None) -> Dict[str, float]:
    ...
    rks_enabled = MODEL_TYPE == "rks"
    rks_gen: RKSFeatureGenerator | None = RKSFeatureGenerator() if rks_enabled else None
    ...
    for chunk in _chunk_dates(train_dates, N_CHUNKS):
        ...
        xdf, ydf = feat_gen.genFeatures(raw)
        del raw
        if rks_gen is not None and not rks_gen._fitted:
            x_base = xdf.to_numpy(dtype=np.float64, copy=False)
            rks_gen.fit(x_base)
        if rks_gen is not None:
            xdf = _add_rks_features(xdf, rks_gen)
