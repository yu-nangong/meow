...
FORECAST_CS_MEAN_SHRINK = float(os.environ.get("MEOW_FORECAST_CS_MEAN_SHRINK", "0.10"))
...
XGB_RESIDUAL_BLEND_WEIGHT = float(os.environ.get("MEOW_XGB_RESIDUAL_BLEND", "0.25"))
XGB_RESIDUAL_ENABLED = os.environ.get("MEOW_XGB_RESIDUAL", "1") != "0"
XGB_RESIDUAL_OBJECTIVE = os.environ.get("MEOW_XGB_RESIDUAL_OBJECTIVE", "rank:pairwise").strip()
XGB_RESIDUAL_N_ESTIMATORS = int(os.environ.get("MEOW_XGB_RESIDUAL_N_ESTIMATORS", "200"))
XGB_RESIDUAL_MAX_DEPTH = int(os.environ.get("MEOW_XGB_RESIDUAL_MAX_DEPTH", "6"))
XGB_RESIDUAL_LEARNING_RATE = float(os.environ.get("MEOW_XGB_RESIDUAL_LEARNING_RATE", "0.05"))
...
def train_and_evaluate(h5dir: Optional[str] = None) -> Dict[str, float]:
    ...
    xgb_residual = None
    if XGB_RESIDUAL_ENABLED:
        os.environ["MEOW_XGB_OBJECTIVE"] = XGB_RESIDUAL_OBJECTIVE
        os.environ["MEOW_XGB_N_ESTIMATORS"] = str(XGB_RESIDUAL_N_ESTIMATORS)
        os.environ["MEOW_XGB_MAX_DEPTH"] = str(XGB_RESIDUAL_MAX_DEPTH)
        os.environ["MEOW_XGB_LEARNING_RATE"] = str(XGB_RESIDUAL_LEARNING_RATE)
        xgb_residual = XGBModel()
        xgb_residual.reset()
        for chunk in _chunk_dates(train_dates, N_CHUNKS):
            raw = pd.concat(list(iter_days(h5dir, chunk)), ignore_index=True)
            xdf, ydf = feat_gen.genFeatures(raw)
            ...
            ydf_resid = ydf.copy()
            ydf_resid.loc[:, "fret12"] = resid
            xgb_residual.partial_fit(xdf, ydf_resid)
            del xdf, ydf, base_pred, resid, ydf_resid
        xgb_residual.finalize_fit()
    ...
    for chunk in _chunk_dates(train_dates, N_CHUNKS):
        ...
        del raw
        base_pred = _postprocess_forecast(ydf, model.predict(xdf), forecast_cs_mean_shrink)
        if xgb_residual is not None:
            base_pred = base_pred + XGB_RESIDUAL_BLEND_WEIGHT * xgb_residual.predict(xdf)
    ...
    if panel_residual.enabled and panel_residual.blend:
        ...
        for chunk in _chunk_dates(panel_dates, N_CHUNKS):
            ...
            if xgb_residual is not None:
                forecast = forecast + XGB_RESIDUAL_BLEND_WEIGHT * xgb_residual.predict(xdf)
    ...
    for chunk in _chunk_dates(test_dates, N_CHUNKS):
        ...
        forecast = _postprocess_forecast(ydf, model.predict(xdf), forecast_cs_mean_shrink)
        if xgb_residual is not None:
            xgb_resid = xgb_residual.predict(xdf)
        else:
            xgb_resid = np.zeros(len(raw), dtype=np.float64)
        forecast = forecast + XGB_RESIDUAL_BLEND_WEIGHT * xgb_resid
        forecast = forecast + interval_residual.predict(xdf, base_pred=forecast)
