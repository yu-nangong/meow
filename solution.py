...
from models.interval_market_residual import IntervalMarketResidualModel
...
def train_and_evaluate(h5dir: Optional[str] = None) -> Dict[str, float]:
    ...
    interval_market_residual = IntervalMarketResidualModel()
    for chunk in _chunk_dates(train_dates, N_CHUNKS):
        raw = pd.concat(list(iter_days(h5dir, chunk)), ignore_index=True)
        xdf, ydf = feat_gen.genFeatures(raw)
        del raw
        ...
        interval_market_residual.partial_fit(xdf, resid, base_pred=base_pred)
        del xdf, ydf, base_pred, resid
    interval_residual.finalize_fit()
    interval_market_residual.finalize_fit()
    ...
    for chunk in _chunk_dates(test_dates, N_CHUNKS):
        ...
        forecast = forecast + interval_market_residual.predict(xdf, base_pred=forecast)
