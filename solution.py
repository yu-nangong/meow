...
RANK_TARGET = os.environ.get("MEOW_RANK_TARGET", "1") != "0"
...
def _train_target_array(ydf: pd.DataFrame) -> np.ndarray:
    ...
    if RANK_TARGET:
        ...
        rank_pct = frame.groupby(["date", "interval"], sort=False)["fret12"].rank(pct=True)
        return (rank_pct.to_numpy(dtype=np.float64, copy=False) - 0.5)
    if TRAIN_ON_INTERVAL_DEMEANED_TARGET:
        frame = pd.DataFrame(
            {
                "date": ydf.index.get_level_values("date"),
                "interval": ydf.index.get_level_values("interval"),
