...
AUX_TARGET_WEIGHT = float(os.environ.get("MEOW_AUX_TARGET_WEIGHT", "0.06"))
...
def train_and_evaluate(h5dir: Optional[str] = None) -> Dict[str, float]:
    ...
    for chunk in _chunk_dates(train_dates, N_CHUNKS):
        ...
        if AUX_TARGET_WEIGHT != 0.0 and AUX_TARGET_MODE != "off":
            aux_pred = _center_by_group(ydf, aux_model.predict(xdf))
            base_pred = base_pred + AUX_TARGET_WEIGHT * aux_pred
    ...
    for chunk in _chunk_dates(test_dates, N_CHUNKS):
        ...
        if AUX_TARGET_WEIGHT != 0.0 and AUX_TARGET_MODE != "off":
            aux_pred = _center_by_group(ydf, aux_model.predict(xdf))
            forecast = forecast + AUX_TARGET_WEIGHT * aux_pred
